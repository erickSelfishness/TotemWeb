...
class FileARBehavior extends CActiveRecordBehavior {
	...
	protected function getExistingFilesName($path, $fname) {
		$files = glob($path.DIRECTORY_SEPARATOR.$fname.'.{'.str_replace(' ', '', $this->extension).'}', GLOB_NOSORT | GLOB_BRACE);
        if ($files == FALSE)
            $files = array();
        return $files;
	}
	 ...
	 */
	public function getFileUrlWithSuffix($suffix) {
		$path = $this->getFolderPath();
	...
	public function afterSave($evt) {
        if (isset($_POST[$this->getDeleteInputName()])){
            $path = $this->getFolderPath();
            $fname = $this->getFileName();
    ...
    public function getDeleteInputName(){
        return 'delfile_'.$this->attribute;
    }
	...
	protected function saveFile($file, $filePath, $fileName, $ext) {
		...
		if (count($this->formats) == 1 && !isset($klass)) {
		    $suffix = $this->formats['normal']['suffix'];
			$file->saveAs($path.$suffix.$real_ext);
		} else if (isset($klass)) {
	...
	protected function deleteFileWithSuffix($path, $fname, $suffix) {
		$fs = $this->getExistingFilesName($path, $fname . $suffix);
		foreach ($fs as $f) {
            unlink($f);
        }
	}    
